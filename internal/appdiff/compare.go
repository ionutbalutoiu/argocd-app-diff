...
func compare(req CompareRequest, printer diffPrinter) (Result, error) {
	...
	groupedTargets := groupObjsByKey(req.DesiredObjects, req.LiveResources, req.Application.Spec.Destination.Namespace)
...
func printDiffItems(items []diffItem, diffConfig argodiff.DiffConfig, printer diffPrinter) (bool, error) {
	...
	for _, item := range items {
		...
		live, target, err := resolveDiffDisplayObjects(item, diffRes.PredictedLive)
		if err != nil {
			return false, err
		}
