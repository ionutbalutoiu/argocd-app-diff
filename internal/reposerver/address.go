...
func ParseURL(input string) (string, repoapiclient.TLSConfiguration, error) {
	...
	if parsed.User != nil {
		return "", repoapiclient.TLSConfiguration{}, fmt.Errorf("repo server URL must not include user info")
	}
	if parsed.Path != "" {
		return "", repoapiclient.TLSConfiguration{}, fmt.Errorf("repo server URL must not include a path")
	}
	if parsed.RawQuery != "" {
		return "", repoapiclient.TLSConfiguration{}, fmt.Errorf("repo server URL must not include query parameters")
	}
	if parsed.Fragment != "" {
		return "", repoapiclient.TLSConfiguration{}, fmt.Errorf("repo server URL must not include a fragment")
	}
